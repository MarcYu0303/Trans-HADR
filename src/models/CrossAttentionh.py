...
class WindowCrossAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0., attention_direction=None):
    ...
    def forward(self, x, mask=None):
        ...
        if self.attention_direction == 'cross':
            x = [branch_1, branch_2]
        elif self.attention_direction == 'forward':
            x = [branch_1, branch_1]
        elif self.attention_direction == 'back':
            x = [branch_2, branch_2]
        else:
...
class SwinCrossAttentionBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=7, shift_size=0,
            ...
            act_layer=nn.GELU, norm_layer=nn.LayerNorm, attention_direction=None):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
        ...
        self.attn = WindowCrossAttention(
            ...
            qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, attention_direction=attention_direction)
...
class CrossAttention(nn.Module):
    def __init__(self,
        ...
        Args:
            in_channel: 输入通道数
            depth: transformer模块重复次数
            num_heads: 注意力头数量
            attention_direction: 进行attention方向，A->B:forward,A提供V,B->A:back，B提供V,A<->B:cross,
        '''
        super().__init__()
        self.cross_attn_layer = BasicCrossAttentionLayer(dim=in_channel, depth=depth, num_heads=num_heads, attention_direction=attention_direction)
