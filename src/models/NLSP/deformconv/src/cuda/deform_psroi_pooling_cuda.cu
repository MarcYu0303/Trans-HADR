...
#include<ATen/ceil_div.h>  //wojiade
#include <ATen/ATen.h>
                                  ...
                                  const float trans_std)
{
  ...
  //dim3 grid(std::min(at::ceil_div(out_size, 512), 4096));
  int64_t grid_size = std::min(at::ceil_div(static_cast<int64_t>(out_size), static_cast<int64_t>(512)), static_cast<int64_t>(4096));
  dim3 grid(grid_size);
