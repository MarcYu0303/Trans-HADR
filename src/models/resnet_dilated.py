...
import models.resnet_adjusted as resnet_adjusted
...
class Resnet34_8s(nn.Module):
    ...
    def __init__(self, inp_ch=4, out_ch=1):
    ...
    def forward(self, x, feature_alignment=False):
        ...
        x = self.resnet34_8s(x) # B, 32, 28, 28 (H/8, W/8)
        
        x = nn.functional.interpolate(input=x, size=input_spatial_dim, mode='bilinear', align_corners=False)

        return x
    
class Resnet34_8s_no_interpolate(nn.Module):
    
    
    def __init__(self, inp_ch=4, out_ch=1):
        
        super(Resnet34_8s_no_interpolate, self).__init__()
